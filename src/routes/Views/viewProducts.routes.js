...
viewProducts.get('/:pid', async (req, res) => {
    let { pid } = req.params
    let page = parseInt(req.query.page);
    if (!page) page = 1;
    let result = await productModel.paginate({id: pid}, { page, limit: 10, lean: true })
    result.prevLink = result.hasPrevPage ? `http://localhost:8080/products/${pid}?page=${result.prevPage}` : '';
    result.nextLink = result.hasNextPage ? `http://localhost:8080/products/${pid}?page=${result.nextPage}` : '';
    result.isValid = !(page <= 0 || page > result.totalPages)
    res.render('products', result);
    console.log(result);
})
